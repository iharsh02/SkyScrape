...
export const ExecutionViewer = ({ initialData }: { initialData: ExecutionData }) => {
  ...
  return (
    <div className="flex flex-col md:flex-row w-full h-full">
      ...
      <aside className={cn(
        "w-full md:w-[400px] md:min-w-[440px] md:max-w-[440px] border-r-2 flex flex-grow flex-col overflow-hidden",
      ...
      <CardContent className="py-4">
        <div className="flex flex-col gap-2">
          ...
          {params && Object.entries(params).map(([key, value]) => (
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2" key={key}>
              <p className="text-sm text-muted-foreground w-full sm:w-1/2">{key}</p>
              <Input
                readOnly
          ...
          <TableBody>
            {logs.map(log => (
              <TableRow key={log.id} className="text-muted-foreground">
                ...
                <TableCell className="text-sm flex-1 p-[3px] pl-4 break-all">
                  {log.message}
                </TableCell>
              </TableRow>
            ))}
